// Reine Textauswertung der Vorbemessungs-PDFs — Feldmuster, Plausibilitäts-
// grenzen und der Aufbau des Dokuments.
//
// Bewusst OHNE pdfjs-Import: dieses Modul soll ohne Vite-Bundler laufen, damit
// die Muster und Grenzen automatisiert getestet werden können.  Das Einlesen
// der PDF-Binärdaten liegt weiterhin in pdfExtract.js.
...
// Plausibilitätsbereiche für die Zahlenfelder.
//
// Die Muster greifen die erste Zahl nach einem Label.  Steht dort z. B.
// "WDVS-Dicke gem. Zulassung Z-1: 140 mm", liefert das Muster die 1 aus
// "Z-1" — ein Wert, der plausibel aussieht und bisher grün als Treffer
// markiert wurde.  Werte außerhalb dieser Bereiche werden daher NICHT
// übernommen, sondern gemeldet, damit der Planer sie selbst einträgt.
export const PLAUSIBEL = {
  wdvs_dicke:        { min: 20, max: 500, einheit: "mm" },
...
// true, wenn der Wert für dieses Feld im erwartbaren Bereich liegt.
export function istPlausibel(key, roh) {
  const g = PLAUSIBEL[key];
  if (!g) return true;                       // Textfeld → keine Prüfung
  const n = parseNum(roh);
  if (!Number.isFinite(n)) return false;
  return n >= g.min && n <= g.max;
}

// Run all patterns, return {values, hits, misses, unplausibel}
export function parseFields(rawText) {
  const text = clean(rawText || "");
  const values = {};
  const hits = [];
  const misses = [];
  const unplausibel = [];                    // {key, wert, min, max, einheit}
  for (const [key, patterns] of Object.entries(PATTERNS)) {
    const v = pickField(text, patterns);
    if (v) {
      if (!istPlausibel(key, v)) {
        const g = PLAUSIBEL[key];
        unplausibel.push({ key, wert: v, ...g });
        misses.push(key);                    // bewusst NICHT übernehmen
        continue;
      }
      values[key] = v;
      hits.push(key);
